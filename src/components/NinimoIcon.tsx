...
export const NinimoIcon: React.FC<NinimoIconProps> = ({ className = '', size = 'md', onClick }) => {
  ...
  // Dynamic colors based on theme - sleek black-ish bot face with vibrant glowing accents across all themes
  const accentColor = isColourUI ? '#10b981' : '#38bdf8';
  const secondaryAccent = isColourUI ? '#34d399' : '#00f0ff';
  const eyeLight = isColourUI ? '#a7f3d0' : '#ffffff';
  const eyeDark = isColourUI ? '#064e3b' : '#09090b';

  // Palette for burst particles
  const particleColors = isColourUI
    ? ['#10b981', '#34d399', '#38bdf8', '#818cf8', '#a7f3d0', '#fbbf24', '#f43f5e']
    : ['#38bdf8', '#00f0ff', '#f4f4f5', '#a1a1aa', '#71717a', '#facc15'];
      ...
      <div
        className={`absolute -inset-1.5 rounded-xl blur-md transition-all duration-300 ${
          isColourUI
            ...
            : 'bg-gradient-to-tr from-zinc-900/80 via-zinc-800/60 to-black/90 opacity-60 group-hover:opacity-90 group-hover:scale-105'
        }`}
      />

      {/* Main Beveled Container */}
      <div
        className={`relative w-full h-full rounded-xl p-[1.5px] shadow-lg border overflow-hidden flex items-center justify-center transition-all duration-200 ${
          isColourUI
            ? 'bg-gradient-to-b from-[#0f172a] via-[#090d16] to-[#04070d] border-emerald-500/40 shadow-emerald-950/40 group-hover:border-emerald-400/80'
            : 'bg-gradient-to-b from-zinc-900 via-zinc-950 to-black border-zinc-800/90 shadow-black/80 group-hover:border-zinc-600'
        }`}
      >
        ...
        <div
          className={`absolute inset-0 [background-size:5px_5px] ${
            isColourUI
              ...
              : 'bg-[radial-gradient(#52525b_1px,transparent_1px)] opacity-30'
          }`}
        />
        ...
        >
          ...
          <rect x="19" y="6" width="2" height="3" fill="#52525b" />
          ...
          <rect x="3" y="15" width="3" height="8" rx="0.5" fill="#27272a" />
          <rect x="3.5" y="17" width="2" height="4" rx="0.5" fill={accentColor} />

          <rect x="34" y="15" width="3" height="8" rx="0.5" fill="#27272a" />
          <rect x="34.5" y="17" width="2" height="4" rx="0.5" fill={accentColor} />

          {/* Main Bot Helmet Chassis (Black-ish Obsidian across ALL themes) */}
          <rect
            x="6"
            y="8"
            width="28"
          ...
          {isWinking ? (
            ...
            <g className="animate-pulse">
              <path
                ...
                stroke={isColourUI ? '#34d399' : '#00f0ff'}
                strokeWidth="2"
                strokeLinecap="round"
                fill="none"
              />
          ...
          <rect x="13.5" y="27.5" width="3.5" height="2" rx="0" fill="#27272a" />
          <rect
            x="18.25"
            y="27.5"
            width="3.5"
          ...
          />
          <rect x="23" y="27.5" width="3.5" height="2" rx="0" fill="#27272a" />
        </svg>
        ...
        <span
          className={`absolute bottom-1 right-1 w-2 h-2 rounded-full transition-all duration-300 ${
            isWinking
              ? 'scale-125 bg-amber-400 shadow-[0_0_10px_rgba(251,191,36,1)] ring-2 ring-zinc-900'
              : isColourUI
              ? 'bg-emerald-400 ring-2 ring-emerald-950 shadow-[0_0_8px_rgba(52,211,153,0.9)] animate-pulse'
              : 'bg-emerald-400 ring-2 ring-zinc-950 shadow-[0_0_6px_rgba(52,211,153,0.8)] animate-pulse'
          }`}
        />
      </div>
    </motion.div>
