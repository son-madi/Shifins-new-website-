...
import { 
  ...
  Keyboard, UserPlus, Package, Crosshair, Crown, Wrench
} from 'lucide-react';
...
export const BotPluginsCard: React.FC<BotPluginsCardProps> = ({ config, isOnline, botId, onUpdateConfig }) => {
  ...
  const PluginRow: React.FC<{
    ...
    isEnabled: boolean;
    badge?: 'PRO' | 'DEV';
  }> = ({ id, icon: Icon, title, description, isEnabled, badge }) => (
    <div className={`flex items-start justify-between p-3 border rounded-xl transition-colors ${
      ...
      <label className="relative inline-flex items-center cursor-pointer shrink-0 mt-1 ml-2">
        <input
          type="checkbox"
          checked={isEnabled}
